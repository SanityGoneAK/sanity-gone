...
export function aggregateModuleData() {
    ...
    Object.entries(cnBattleEquipTable).forEach(([moduleId, cnBattleEquip]) => {
        ...
        const moduleName = Object.keys(UNIEQUIP_LOCALES).reduce((locales, locale) => {
            if (UNIEQUIP_LOCALES[locale].equipDict[moduleId]){
                locales[locale] = UNIEQUIP_LOCALES[locale].equipDict[moduleId].uniEquipName;
            }

            if (locale == 'en_US' && !UNIEQUIP_LOCALES[locale].equipDict[moduleId] && moduleId in moduleTranslations && moduleTranslations[moduleId].moduleName){
        ...
        operatorBattleEquip.phases.forEach((currentPhase, i) => {
                ...
                if (
                    ...
                    for (let k = 0; k < traitCandidates.length; k++) {
                        ...
                        if (curTraitCandidate.additionalDescription) {
                            candidates[curTraitCandidate.requiredPotentialRank].traitEffect =
                                Object.keys(BATTLE_EQUIP_LOCALES).reduce((locales, locale) => {
                                    if (!BATTLE_EQUIP_LOCALES[locale][moduleId]){
                                        console.warn(`CN Only module: ${moduleId} at locale: ${locale}`)
                                        return locales;
                                    }

                                    const localizedCandidate = BATTLE_EQUIP_LOCALES[locale][moduleId].phases[i].parts[j].overrideTraitDataBundle.candidates[k]
                            ...
                            candidates[curTraitCandidate.requiredPotentialRank].traitEffectType = "update";
                            ...
                            candidates[curTraitCandidate.requiredPotentialRank].traitEffectType = "override";
                        }
                    }
                    ...
                    for (let k = 0; k < talentCandidates.length; k++) {
                        ...
                        if (curTalentCandidate.displayRangeId) {
                            candidates[curTalentCandidate.requiredPotentialRank].range = rangeTable[curTalentCandidate.rangeId];
                            candidates[curTalentCandidate.requiredPotentialRank].displayRange = true;
                        }
                        ...
                        if (curTalentCandidate.upgradeDescription) {
                            candidates[curTalentCandidate.requiredPotentialRank].talentEffect =
                                Object.keys(BATTLE_EQUIP_LOCALES).reduce((locales, locale) => {
                                    if (!BATTLE_EQUIP_LOCALES[locale][moduleId]){
                                        console.warn(`CN Only module: ${moduleId} at locale: ${locale}`)
                                        return locales;
                                    ...
                                    const localizedCandidate = BATTLE_EQUIP_LOCALES[locale][moduleId].phases[i].parts[j].addOrOverrideTalentDataBundle.candidates[k];

                                    // Random korean Typo not sure
                                    if (locale == 'ko_KR' && moduleId == 'uniequip_002_hamoni' && j == 1 &&  k == 1){
                                        localizedCandidate.upgradeDescription = localizedCandidate.upgradeDescription.includes('</>') ? localizedCandidate.upgradeDescription : localizedCandidate.upgradeDescription + '</>'
                                    }
            ...
            if (moduleId in moduleTranslations) {
                for (let j = 0; j < moduleTranslations[moduleId].phases.length; j++) {
                    ...
                    if (
                        ...
                        candidates[
                            moduleTranslations[moduleId].phases[j].requiredPotentialRank].traitEffect['en_US'] = moduleTranslations[moduleId].phases[j].translations[i].trait;
                        candidates[
                            moduleTranslations[moduleId].phases[j].requiredPotentialRank].talentEffect['en_US'] = moduleTranslations[moduleId].phases[j].translations[i].talent;
                    }
                }
            }
            ...
            if (
                (candidates[0].talentEffect && Object.keys(candidates[0].talentEffect).length <= 1) ||
                (candidates[0].traitEffect && Object.keys(candidates[0].traitEffect).length <= 1)
            ) {
                console.warn(
