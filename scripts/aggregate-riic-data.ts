...
export async function aggregateRiicData() {
    ...
    Object.values(BUILDING_LOCALES.zh_CN.chars)
        .filter((buffChar) => {
            ...
            const enCharRiicData = BUILDING_LOCALES.en_US.chars[charId as keyof typeof BUILDING_LOCALES.en_US.chars];
                    ...
                    const buffStages = buffData.map(
                        (cnBuff, skillLevelIndex) => {
                            ...
                            if (enCharRiicData != null) {
                                const enBuff = enCharRiicData.buffChar[skillIndex].buffData[skillLevelIndex];
                            ...
                            Object.keys(BUILDING_LOCALES).forEach(locale => {
                                const buffData = BUILDING_LOCALES[locale].buffs[buffId as keyof typeof BUILDING_LOCALES[locale]['buffs']]
                                ...
                                if ((requiresJetTL || buffData == null) && jetTL && locale == "en_US") {
                                    name[locale] = jetTL.name;
                                    description[locale] = jetTL.description;
                                    return;
                                }

                                console.warn("No translation available for buff: " + buffId + " in locale: " + locale);
                            })
                            ...
                            return {
                                ...
                                name,
                                description,
                                skillIcon: BUILDING_LOCALES.zh_CN.buffs[buffId as keyof BUILDING_LOCALES.zh_CN.buffs].skillIcon,
                            };
                        }
                    );
