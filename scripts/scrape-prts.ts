		...
		await Promise.all(
			brandLinks.map(async (brandLink) => {
				...
				const brandSkinEntries = $("h2 + .wikitable").map((_i, el) => {
					...
					cnOriginalObtainSources.forEach(
						(cnOriginalObtainSource) => {
							switch (cnOriginalObtainSource) {
								case "机密圣所":
								case "危机合约":
									allObtainSources.add(
										SkinSource.ContingencyContractStore
									);
									break;
								case "采购中心":
									allObtainSources.add(
										SkinSource.OutfitStore
									);
									break;
								case "兑换码":
								case "特典兑换":
									allObtainSources.add(
										SkinSource.RedemptionCode
									);
									break;
								case "集成战略":
									allObtainSources.add(
										SkinSource.IntegratedStrategies
									);
									break;
								case "活动获得":
					...
					if (allObtainSources.has(SkinSource.OutfitStore)) {
						...
						const mostRecentCostTh = $(el)
							...
							.last()
							.closest("th");
						const rawCostString = mostRecentCostTh
							.find("> div")
							.text()
							.trim();
					...
					if (contingencyContractCostTh.length) {
						...
						allObtainSources.has(
							SkinSource.ContingencyContractStore
						)
					) {
						// uh oh, it's a CC skin but we couldn't find the cost
						console.warn(
							`Couldn't find cost for CC skin: ${cnSkinName}`
						);
					}
