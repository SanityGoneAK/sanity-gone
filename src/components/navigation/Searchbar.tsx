...
import { operatorAvatar, operatorBranchIcon, operatorClassIcon } from "../../utils/images.ts";
...
const CustomSearchInput: React.FC<{ placeholder: string,  inputRef: React.RefObject<HTMLInputElement> }> = ({
    placeholder,
    inputRef
}) => {
    ...
    const handleOptionSelected = useCallback(
        (option: SearchResult | null) => {
            ...
            if (option.type === "operator") {
                window.location.href = `/operators/${slugify(option.name.en_US ?? '')}`;
                    ...
                    {operatorResults.slice(0, 5).map(result => (
                        ...
                        >
                            <img
                                ...
                                height={40}
                            />
                            <span className="text-neutral-100">{result.name.en_US}</span>
                            <p className="text-sm leading-[18px] text-neutral-200">
                                <span className={cx('inline-block w-6 text-neutral-100', rarityClasses[result.rarity as keyof typeof rarityClasses])}>{result.rarity}★</span>
                                <span>{result.class}&nbsp; •&nbsp; {result.subclass.en_US}</span>
                            </p>
                        </Combobox.Option>
                    ))}
                </ul>
                    ...
                    {classResults.slice(0, 3).map((result) => (
                        ...
                        >
                            ...
                            <span className="text-neutral-100">
                                {result.type === "class" ? result.name : result.name.en_US}
                            </span>
            ...
            >
                ...
                <Combobox<SearchResult>>
                    {({ activeOption }) => (
                        ...
                        >
                            <CustomSearchInput placeholder={placeholder} inputRef={inputRef} />
